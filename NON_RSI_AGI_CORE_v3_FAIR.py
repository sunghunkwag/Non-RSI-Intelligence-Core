...
class Agent:
    ...
    def choose_action(self, obs: Dict[str, Any]) -> str:
        ...
        # System 2: Meta-cognitive reflection (v3 upgrade)
        # Check memory for past failures with this action in similar contexts
        failure_memories = self.mem.search(
            query=f"{obs.get('task', '')} {draft_action} failure",
            k=3,
            kinds=["episode"]
        )
        ...
        for mem in failure_memories:
            if mem.content.get("action") == draft_action:
                ...
                if reward < 0.15:  # Low-reward threshold
                    risk_penalty += 0.3
        
        # If high risk detected, consider alternative actions
        if risk_penalty > self.cfg.risk and len(candidates) > 1:
            # Override with safer alternative
            return candidates[1].actions[0] if len(candidates) > 1 else random.choice(self.action_space())
